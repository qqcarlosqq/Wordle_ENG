/* Wordle Solver — English  (v5.1 • pattern colours + “options” column) */
...
function ensureBody(id){const t=$(id);let tb=t.querySelector('tbody');
  if(!tb){tb=document.createElement('tbody');t.appendChild(tb);}return tb;}
...
function resetAll(){history=[];patterns=[];candidates=[];
  calcDone=false;cacheH={};version++;
  buildColorSelectors(); clearTables(); renderFreqTable(); toggleCompareBtn();}
function onSuggest(){if(!calcDone){updateCandidates();calcDone=true;}
  renderAll(); toggleCompareBtn();}
function updateHist(){const m=['gray','yellow','green'];
  $('history').textContent=history.map((w,i)=>`${w} → ${patterns[i].map(c=>m[c]).join(', ')}`).join('\n');}
...
function renderFreqTable(){
  ...
  for(const w of candidates){
    const seen={}; for(const ch of w){const r=rows[LETTERS.indexOf(ch)];
...
function runCompare(){
  ...
  /* matriz de patrones y agrupaciones */
  const pat = words.map(g=>words.map(s=>patternKey(s,g)));

  /* colores */
  const palette=['#ffd24d','#ffe680','#ffbf00','#b3d9ff','#80bfff',
                 '#4da6ff','#ff9999','#ff6666','#ff4d4d','#b3ffb3'];
  ...
  for(let i=0;i<n;i++){
    const row=pat[i];
    /* mapa patrón -> índice de color + lista de columnas */
    const map={}; row.forEach((p,idx)=>{ if(!map[p]) map[p]={cols:[],clr:null};
      map[p].cols.push(idx);});
    /* asignar colores sólo a grupos de tamaño≥2 */
    let colourIdx=0; Object.values(map).forEach(o=>{
      if(o.cols.length>=2){o.clr=palette[colourIdx%palette.length]; colourIdx++;}});
    let zeros=0;
    html+=`<tr><th>${words[i]}</th>`;
    for(let j=0;j<n;j++){
      const patt=row[j], group=map[patt];
      const next=group.cols.find(c=>c>j); const jump=next?next-j:0;
      if(jump===0) zeros++;
      const bg=group.clr?group.clr:'#f2f2f2';
